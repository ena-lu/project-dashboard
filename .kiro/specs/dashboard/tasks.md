...
**驗收標準（對應 US-01 ～ US-07）：**
- [ ] US-01：頁面載入後顯示所有 8 筆專案
- [ ] US-02：每筆專案顯示正確顏色的狀態 badge
- [ ] US-03：下拉選單包含「全部客戶」＋所有不重複客戶名稱；選擇後正確篩選
- [ ] US-04：展開後可見各階段預計完成日期（來自模擬資料）
- [ ] US-05：`input[type="date"]` 可手動輸入，填入後顯示在欄位中
- [ ] US-06：預估工時顯示模擬資料值；`input[type="number"]` 可輸入
- [ ] US-07：輸入實際工時後，差異立即更新；超時紅色加+號、提早綠色、未填顯示 `—`
- [ ] SVG 趨勢圖在展開的卡片中可見，預估/實際長條可分辨
- [ ] 不引入任何外部套件（無 `import`、無 `<script src="https://...">`)
